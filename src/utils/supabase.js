...
// Function to add to waitlist with duplicate protection
export async function addToWaitlist(email, userId) {
  const { data, error } = await supabase.from("waitlist").insert([
    {
      email: email,
  ...
  if (error) {
    // Supabase uses error code 23505 for unique constraint violation
    if (error.code === "23505") {
      return {
        data: null,
        error: {
          message: "This email has already been used to join the waitlist.",
        },
      };
    }
